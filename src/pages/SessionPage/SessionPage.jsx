  ...
  const handleSaveSession = async () => {
    try {
      ...
      const totalCaloriesBurned = exercises.reduce(
        (total, exercise) => total + (exercise.calories_burned || 0), 0
      );
      const totalReps = exercises.reduce(
        (total, exercise) => total + (exercise.reps || 0), 0
      );
      const totalSets = exercises.reduce(
        (total, exercise) => total + (exercise.sets || 0), 0
      );
      const totalHours = exercises.reduce(
        (total, exercise) => total + ((exercise.duration || 0) / 60), 0 // convert minutes to hours
      );
      
      const uniqueWorkoutTypes = [...new Set(exercises.map((exercise) => exercise.workout_type))];
      const uniqueBodyParts = [...new Set(exercises.map((exercise) => exercise.body_part))];
  ...
  return (
    <main className="session-page">
        ...
        {exercises.length > 0 ? (
          <div className="session-page__exercise-list">
            {exercises.map((exercise) => (
              <div key={exercise.id} className="session-page__exercise-card current-session">
                <h3 className="session-page__exercise-name">{exercise.name}</h3>
                <div className="session-page__exercise-toggle" onClick={() => handleDelete(exercise.id)}>
                  <span>−</span>
                </div>
              </div>
            ))}
            ...
            <div className="session-page__available-exercises">
              {allExercises[workoutType].map((exercise) => (
                <div key={exercise.id} className="session-page__exercise-card">
                  <h3 className="session-page__exercise-card-title">{exercise.name}</h3>
                  <p className="session-page__exercise-card-details">Body Part: {exercise.body_part}</p>
                  <p className="session-page__exercise-card-details">Sets: {exercise.sets}</p>
                  <p className="session-page__exercise-card-details">Reps: {exercise.reps}</p>
                  <p className="session-page__exercise-card-details">Duration: {exercise.duration} minutes</p>
                  <p className="session-page__exercise-card-details">Calories Burned: {exercise.calories_burned}</p>
                  <div className="session-page__exercise-toggle" onClick={() => handleAddExercise(exercise.id)}>
                    <span>+</span>
                  </div>
                </div>
              ))}
      ...
      <div className="session-page__save-session-container">
        <button className="session-page__save-session" onClick={handleSaveSession}>
